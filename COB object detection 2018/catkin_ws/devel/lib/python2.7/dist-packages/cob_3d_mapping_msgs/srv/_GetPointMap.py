...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      _x = self
      buff.write(_struct_2I.pack(_x.map.height, _x.map.width))
      ...
      _x = self
      buff.write(_struct_B2I.pack(_x.map.is_bigendian, _x.map.point_step, _x.map.row_step))
      _x = self.map.data
      length = len(_x)
      ...
      if type(_x) in [list, tuple]:
        ...
        buff.write(struct.pack('<I%ss'%length, length, _x))
      buff.write(_struct_B.pack(self.map.is_dense))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      (_x.map.height, _x.map.width,) = _struct_2I.unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      self.map.fields = []
      ...
      (_x.map.is_bigendian, _x.map.point_step, _x.map.row_step,) = _struct_B2I.unpack(str[start:end])
      ...
      (self.map.is_dense,) = _struct_B.unpack(str[start:end])
